...
class quantite_butane_t(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Quantité extraite de butane (par tonne nette livrée)"
    reference = "https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000006294877/2020-03-25"  # noqa: E501


class redevance_communale_des_mines_butane(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Redevance communale des mines pour le butane"
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        tarif_rcm = parameters(period).redevances.communales.butane

        annee_production = period.last_year
        quantite = articles("quantite_butane_t", annee_production)
...
class redevance_departementale_des_mines_butane(Variable):
    value_type = float
    entity = Article
    label = "Redevance départementale des mines pour le butane"
    definition_period = YEAR
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        tarif_rdm = parameters(period).redevances.departementales.butane
