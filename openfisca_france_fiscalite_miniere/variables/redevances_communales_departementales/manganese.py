...
class quantite_manganese_100t(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Quantité extraite de minerai de manganèse (par centaine de tonnes de manganèse contenu)"  # noqa: E501
    reference = "https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000006294877/2020-03-25"  # noqa: E501


class redevance_communale_des_mines_manganese(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Redevance communale des mines pour le minerai de manganèse"
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        ...
        quantite = articles("quantite_manganese_100t", annee_production)

        tarif_rcm = parameters(period).redevances.communales.manganese

        rcm = (quantite * tarif_rcm) * surface_communale_proportionnee
        return round(rcm, decimals = 2)


class redevance_departementale_des_mines_manganese(Variable):
    value_type = float
    entity = Article
    label = "Redevance départementale des mines pour le minerai de manganèse"
    definition_period = YEAR
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        ...
        tarif_rdm = parameters(period).redevances.departementales.manganese
