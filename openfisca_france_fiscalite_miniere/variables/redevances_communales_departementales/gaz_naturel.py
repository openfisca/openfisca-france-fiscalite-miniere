...
class quantite_gaz_naturel_100km3(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Quantité extraite de gaz naturel (par 100 000 m3  extraits à 1 bar et 15°C)"  # noqa: E501
    reference = "https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000006294877/2020-03-25"  # noqa: E501


class redevance_communale_des_mines_gaz_naturel(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Redevance communale des mines pour le gaz naturel"
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        ...
        quantite = articles("quantite_gaz_naturel_100km3", annee_production)

        tarif_rcm = parameters(period).redevances.communales.gaz_naturel

        rcm = (quantite * tarif_rcm) * surface_communale_proportionnee
        return round(rcm, decimals = 2)


class redevance_departementale_des_mines_gaz_naturel(Variable):
    value_type = float
    entity = Article
    label = "Redevance départementale des mines pour le gaz naturel"
    definition_period = YEAR
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        ...
        tarif_rdm = parameters(period).redevances.departementales.gaz_naturel
