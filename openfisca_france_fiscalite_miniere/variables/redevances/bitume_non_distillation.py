...
class quantite_bitume_non_distillation_kt(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Quantité extraite de calcaires et grès bitumineux ou asphaltiques non destinés à la distillation pour production d'huiles ou d'essences (par millier de tonnes nettes livrées)"  # noqa: E501
    reference = "https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000006294877/2020-03-25"  # noqa: E501


class redevance_communale_des_mines_bitume_non_distillation(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Redevance communale des mines pour calcaires et grès bitumineux ou asphaltiques non destinés à la distillation"  # noqa: E501
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        tarif_rcm = parameters(period).redevances.communales.bitume_non_distillation

        annee_production = period.last_year
        quantite = articles("quantite_bitume_non_distillation_kt", annee_production)
...
class redevance_departementale_des_mines_bitume_non_distillation(Variable):
    value_type = float
    entity = Article
    label = "Redevance départementale des mines pour calcaires et grès bitumineux ou asphaltiques non destinés à la distillation"  # noqa: E501
    definition_period = YEAR
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        tarif_rdm = parameters(
            period).redevances.departementales.bitume_non_distillation
