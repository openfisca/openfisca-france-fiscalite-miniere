...
class quantite_tungstene_t(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Quantité extraite de minerai de tungstène (par tonne d'oxyde de tungstène WO3 contenu)"  # noqa: E501
    reference = "https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000006294877/2020-03-25"  # noqa: E501


class redevance_communale_des_mines_tungstene(Variable):
    value_type = float
    entity = Article
    definition_period = YEAR
    label = "Redevance communale des mines pour le minerai de tungstène"
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        ...
        quantite = articles("quantite_tungstene_t", annee_production)

        tarif_rcm = parameters(period).redevances.communales.tungstene

        rcm = (quantite * tarif_rcm) * surface_communale_proportionnee
        return round(rcm, decimals = 2)


class redevance_departementale_des_mines_tungstene(Variable):
    value_type = float
    entity = Article
    label = "Redevance départementale des mines pour le minerai de tungstène"
    definition_period = YEAR
    reference = [
    ...
    def formula_2020_01(articles, period, parameters) -> ndarray:
        ...
        tarif_rdm = parameters(period).redevances.departementales.tungstene
